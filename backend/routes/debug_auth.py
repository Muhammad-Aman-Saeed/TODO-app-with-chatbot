...
def login(login_data: LoginRequest):
    ...
    """
    print("Login endpoint called")  # Debug print
    ...
    print(f"Returning token for user: {user_data['id']}")  # Debug print
...
def register(register_data: RegisterRequest):
    ...
    # Create a JWT token with user information
    user_data = {
        "id": f"user-{abs(hash(register_data.email)) % 10000}",  # Generate a unique ID based on email
...
def get_auth_token():
    """
    Get auth token endpoint - returns dummy data for testing
    """
    print("Token endpoint called")  # Debug print
